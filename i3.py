...
silent_mode = False 
about = False
install_powershell_tools = False
...
def get_user_input(prompt:str) -> bool:
  ...
  try:
    response = messagebox.askokcancel('askquestion', prompt)
  except Exception as e:
...
def configure_installed() -> bool:
  ...
  for package_name in requires_config:
    if get_user_input(f"Open configuration for {package_name}"):
